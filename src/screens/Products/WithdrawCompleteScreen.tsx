...
export default function WithdrawCompleteScreen({
  ...
  const isSimulated = txHash.includes('test') || (txHash.length === 66 && !txHash.startsWith('0x0'));
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Animated.View style={[styles.infoContainer, { opacity: contentOpacity }]}>
          <Text style={[styles.title, { color: colors.textPrimary }]}>해지 완료!</Text>
          ...
          <Card variant="dark" style={styles.summaryCard}>
            ...
            <SummaryRow label="해지 금액" value={`${formatUSD(amount)} ${asset}`} highlight />
            <SummaryRow label="수령 자산" value={`${asset} (지갑으로 전송)`} isLast />
          </Card>
